...
File       : puma_school_analysis_1990.do
Purpose    : Analyze relationship between 1990 demographics and private school access
Author     : Myles Owens
...
Description:
    ...
    relationship between 1990 PUMA-level demographic characteristics and private
...
Inputs:
    - school_count_final_1990.dta    Final merged dataset with 1990 demographics + schools

Outputs:
    - summary_stats_1990.doc              Summary statistics for all variables
    - reg_1990_total_school.tex           Regression results using total schools
    - reg_1990_non_relig.tex              Regression results using non-religious schools
    - reg_1990_relig.tex                  Regression results using religious schools
    - reg_1990_distinct_fine.tex          Regression results using fine school diversity
    - reg_1990_distinct_collapsed.tex     Regression results using collapsed school diversity
...
* Load final merged dataset with 1990 demographics and school exposure
use school_count_final_1990, clear
...
* This provides descriptive overview of the 1990 data before regression analysis
...
outreg2 using summary_stats_1990.doc, sum(log) replace
...
* with private school access in 1990.
...
foreach exp in total_school non_relig relig distinct_fine distinct_collapsed {
    ...
    * style(tex): Format for LaTeX
    esttab using "$florida/output/charts/reg_1990_`exp'.tex", replace ///
        se r2 ar2 label style(tex) ///
        title("Regression Results using `exp' (1990)")
}

*** ---------------------------------------------------------------------------
*** Section 5: Multivariate Regression
*** ---------------------------------------------------------------------------

* Multivariate specification with all demographic controls
