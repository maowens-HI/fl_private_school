...
File       : blockxpuma_1990.do
Purpose    : Link block-level school counts to 1990 PUMAs via geographic crosswalk
Author     : Myles Owens
Institution: Hoover Institution, Stanford University
Date       : 2025-12-01
───────────────────────────────────────────────────────────────────────────────
Description:
    This script links Census block-level private school counts to 1990 PUMA-level
    demographics. It processes a 1990 PUMA × Block geographic crosswalk file,
...
Outputs:
    - blockxpuma_clean_1990.dta      Cleaned geographic crosswalk with baseid
    - pumaxblock_school_all_1990.dta PUMA-level school counts (aggregated)
    - school_count_final_1990.dta    Final merged dataset (demographics + schools)
...
Notes:
    - Uses 1990 PUMA boundaries (different from 2000!)
...
* Load 1990 PUMA × Block crosswalk
...
tostring censusblock, gen(block) force
...
save blockxpuma_clean_1990.dta, replace
...
use blockxpuma_clean_1990, clear
...
save pumaxblock_school_all_1990, replace

*** ---------------------------------------------------------------------------
*** Section 4: Merge Demographics with School Exposure
*** ---------------------------------------------------------------------------

* Load PUMA-level demographic characteristics (from fl_demo_reg_1990.do)
use fl_puma1990_analysis.dta, clear

* Merge with PUMA-level school counts (one-to-many: one PUMA, many distances)
merge 1:m puma5 using pumaxblock_school_all_1990
...
save school_count_final_1990, replace
