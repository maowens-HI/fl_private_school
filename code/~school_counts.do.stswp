...
File       : school_count.do
Purpose    : Construct block-level count of all schools at different radii
Author     : Myles Owens
...
Description:
    ...
    Census block centroid in Florida. The analysis is performed separately for
    three school levels (elementary, middle, high school) and generates counts
    by school type (religious vs. non-religious) and type diversity measures.
...
Inputs:
    - fl_blocks2000_centroids.dta    Block centroids with lat/lon coordinates
...
* Start with HIGH
use "block_school_counts_all_high.dta", clear
gen level = "high"

* Append MIDDLE
append using "block_school_counts_all_middle.dta"
replace level = "middle" if missing(level)

* Append ELEM
append using "block_school_counts_all_elem.dta"
replace level = "elem" if missing(level)

save "block_school_counts_all.dta", replace
